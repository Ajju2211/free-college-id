...
function CollegeForm(props) {
  ...
  if (authRequired && !isLogged) {
    return (
      ...
      >
        <div
          className={styles.signInBox + " card p-0"}
          style={{ width: "50%" }}
        >
          ...
          >
            <p>
              College email does not open in few inbuilt browsers of older
              versions of mobile, use either external browser or desktop to
              signIn in such issues.
            </p>
            <GoogleLogin
              clientId={clientId}
